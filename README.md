...
Group project, Macroeconometrics I — ISEG Lisbon (with Michael Kazibwe and Wilson da Conceição Fernando)

## The question

Everyone treats the Phillips curve as basically dead at this point — the inflation-unemployment tradeoff that's supposed to be central to macro just doesn't show up cleanly in the data anymore. We wanted to check whether a specific shock, the 2018 US-China trade war, actually moved the needle on that relationship in the five BRICS economies, or whether it's just noise like everything else.
...
Every model gets run per country (not pooled), then corrected with Newey-West HAC standard errors once we'd checked for heteroskedasticity and autocorrelation. We also ran Chow and Quandt-Andrews tests to check for a clean structural break at the trade war date, and unit root tests up front to make sure everything was stationary.

One methodological choice worth flagging: we used the raw unemployment rate rather than an unemployment gap (which would need a NAIRU estimate via HP or Kalman filter) — for BRICS labour markets that's a more standard approach in the literature anyway.
...
So the effect wasn't uniform across the bloc — it shows up specifically in the two economies most directly exposed to the US-China trade relationship, and it works through different channels in each.
...
library(zoo); library(patchwork); library(tseries); library(strucchange); library(urca)

source("Phillip_curve_brics.R")
```
